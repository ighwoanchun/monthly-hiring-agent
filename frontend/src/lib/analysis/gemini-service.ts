...
${data.has_hire_detail ? `#### 4-3-1. 단계별 소요일 분포 (실측 개인별 날짜 기반)
> 📌 데이터 섹션의 "[단계별 소요일 분포]" 테이블을 그대로 복사하여 출력하세요. 테이블 위아래 빈 줄 필수.

💡 **분포 인사이트**: 평균과 중앙값 차이로 분포 편향을, 90분위로 장기 지연 합격자 존재 여부를 분석. 예: "평균 ${month}일이지만 중앙값은 N일로, 일부 장기 전형이 평균을 끌어올림"
` : ""}
